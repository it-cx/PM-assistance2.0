...
| 指标 | 衡量方式 | MVP 目标 |
|------|---------|:---:|
| 任务完成率 | 用户发起查询/诊断/操作请求后 Agent 成功返回有效结果的比例 | ≥ 80% |
| 诊断效率 | 从用户输入 VIN+问题到 Agent 输出诊断结论的平均耗时 | < 3 分钟 |
| 诊断质量 | 远程诊断结论与现场确认结果的吻合率 | ≥ 70% |
| 告警价值 | 用户收到告警推送后在会话中追问的比例（非已读不回） | ≥ 30% |
...
Agent 在分析过程中自动检索故障模式库、部件规格、供应商对照等知识库内容，辅助诊断。对话结束时，如产生有价值的诊断结论且不在现有知识库中，Agent 可提议"是否需要生成知识条目草稿？"，用户同意后生成，走专人审核流程入库。
...
飞书会话
    ...
    │                      返回结果 → 格式化 → 飞书回复
    │
    ├── 系统告警 → 消息网关 → Agent 补全上下文 → 飞书推送
    │
    └── 定时 tick → 巡检触发 → 扫描 → 报告推送
```
...
| 类型 | 范围 | 内容 |
|------|------|------|
| **短期记忆** | 当前会话 | 用户身份、租户 scope、认证 token、本轮对话中已查询过的车辆/批次/指标、当前确认等待状态 |
| **长期记忆** | 不跨用户，不跨会话 | 不做——所有历史上下文在新会话中重建 |

### 上下文窗口策略

- 单次会话中，已查询过的车辆、对比过的批次等上下文自然保持在对话历史中
- 如会话超过 20 轮，Agent 可主动提示"当前对话较长，是否需要总结当前分析进度？"
- 不跨会话记忆——每次新会话从零开始，用户登录后重新绑定身份
...
飞书（交互层）
    ...
    ▼
Agent 平台（大脑）
    · 消息网关：飞书消息 / 系统告警 / 定时 tick → 标准化事件
    · 会话管理：用户身份 + 租户 scope + 操作状态跟踪
    · 意图路由：查询 / 分析 / 控制 / 知识
    │ MCP 协议
    ▼
MCP Server（手脚 · 研发实现）
...
| 2 | 工具协议 | MCP，由研发设计实现 |
| 3 | 认证方式 | 用户输入账号密码 → Agent 会话绑定 token → 透传 BFF |
...
Agent 调用认证接口 → 获取 token → 绑定到会话上下文
    │
    ▼
后续每次 MCP 工具调用 → 带 token → BFF → 验证身份 + 注入租户 scope → 执行
    │
    ▼
...
| 指标 | 定义 | MVP 目标 |
|------|------|:---:|
| 查询准确率 | 单维度数据查询返回结果与实际数据一致的比例 | ≥ 95% |
| 诊断吻合率 | 远程诊断结论与现场确认结果的吻合比例 | ≥ 70%（一期允许推测性结论被修正） |
| 知识检索命中率 | 分析过程中知识库返回了相关模式的比例（在知识库已覆盖该问题时） | ≥ 80% |
| 未覆盖率 | 提问中因数据/能力不足无法回答的比例 | < 30% |
...
| 指标 | 定义 | MVP 目标 |
|------|------|:---:|
| 首次响应时间 | 用户发送消息到 Agent 返回首条内容的时间 | < 5 秒 |
| 诊断总耗时 | 从用户输入 VIN+问题到 Agent 输出完整诊断结论 | < 3 分钟 |
| 平均对话轮次 | 完成一次典型诊断所需的对话轮数 | ≤ 5 轮 |

### 运营指标

| 指标 | 定义 | MVP 目标 |
|------|------|:---:|
| 周活跃用户数 | 每周至少使用 1 次的独立用户数 | — |
| 周调用次数 | 每周主动 @机器人 的次数 | ≥ 15 次 |
| 告警追问率 | 用户收到告警推送后在会话中追问的比例 | ≥ 30% |
| 知识条目增长 | 新增故障模式数量 | P1 结束时 ≥ 30 条 |
...
| 诊断总耗时 | < 3 分钟 | 必须达标 |
| 未覆盖率 | < 30% | 必须达标 |
| 诊断吻合率 | ≥ 70% | 一期参考，不作为硬性卡点 |
| 无严重安全缺陷 | 确认流程不可绕过、租户隔离不可绕过 | 一票否决 |
