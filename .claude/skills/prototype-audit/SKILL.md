...
### Fix Phase: Polish with make-interfaces-feel-better

After objective issues are resolved (`pass: true`), run the **make-interfaces-feel-better** skill as a polish pass. Where this skill catches measurable violations, make-interfaces-feel-better catches the details that compound into a great experience:

| Audit catches | make-interfaces-feel-better catches |
|---|---|
| Contrast below WCAG AA | Font smoothing, text-wrap balance/pretty |
| Truncation / overflow clipping | Concentric border radius on nested elements |
| Zero-size collapsed containers | Optical vs geometric icon alignment |
| Unstyled browser defaults | Missing staggered enter/exit animations |
| Sibling overlap | Shadows instead of hard borders |
| Uncaught JS / console errors | Tabular numbers for dynamic values |
| Resource 4xx/5xx errors | Image outlines for depth, minimum hit areas |

Use the audit's `suggestion` field as the first fix pass, then apply make-interfaces-feel-better's principles as the polish pass. The two skills together form a complete **check → fix → polish** pipeline.
...
- ❌ Aesthetic judgments — "this looks ugly", "this color is unappealing"
  - → Use **make-interfaces-feel-better** for subjective polish (border radius, animations, shadows, typography)
